...
def distance_matrix_from_jax_implementation(a,b,squared: bool = False):
    aa = torch.einsum("nd,nd->n", a, a)
    bb = torch.einsum("md,md->m", b, b)
    ab = torch.einsum("nd,md->nm", a, b)

    dist_sqr = aa[:, None] + bb[None, :] - 2 * ab
    # protection against numerical errors resulting in NaN
    dist_sqr = torch.maximum(dist_sqr, 0.0)

    if squared:
        return dist_sqr
    else:
        return torch.sqrt(dist_sqr)
...
def batched_pairwise_distance(a, b, distance_fn, block_size=16):
...
class BenchmarkCallback:
    ...
    def _assemble_dist_m(self, ss_dist, sd_dist):
        ...
        return torch.concatenate(
            [
                torch.concatenate([ss_dist, sd_dist], axis=1),
                torch.concatenate([ds_dist, dd_dist], axis=1),
            ],
            axis=0,
        )
    ...
    def _one_nn_acc(self, ss_dist, sd_dist):
        ...
        n = ss_dist.shape[0]
        torch.fill_diagonal(dist_m, float("inf"))

        amin = dist_m.argmin(axis=0)
        one_nn_1 = amin[:n] <= n
        one_nn_2 = amin[n:] > n

        return torch.concatenate([one_nn_1, one_nn_2]).mean()

    def _mmd(self, sd_dist):
        return sd_dist.min(axis=0).min()

    def _cov(self, sd_dist):
        return torch.unique(sd_dist.argmin(axis=1)).size / sd_dist.shape[1]
