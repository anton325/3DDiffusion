...
from gecco_torch.additional_metrics.metrics_so3 import c2st_gaussian, minimum_distance, rotational_distance_between_pairs_dot_product
...
flags.DEFINE_string("dataset", "gaus_rot_x0", "Dataset to train on. Can be 'checkerboard'.")
...
flags.DEFINE_string("sampler", "heun", "Sampler to use. Can be 'original' or 'heun'.") # heun original_sample ddpm_sampling
...
@torch.no_grad()
def sample_heun_so3_alt(model,gt_rotations,noise_schedule):
    print("heun sampling ...")
...
def sample_heun_so3(model,gt_rotations,noise_schedule):
    ...
    reverse_schedule = torch.flip(noise_schedule,[0])
    x_t = noisy_rotation
    ...
    for i, (t_cur, t_next) in ts:
        ...
        # plt.savefig(f"t_{i}_2.png")
        step_size = t_next - t_cur
        noise_level = t_cur * torch.ones([x_t.shape[0],1],device=x_t.device)
        mu, scale = model(x_t, noise_level)
        # visualize_so3_probabilities(jnp.array([jaxlie.SO3(x[[3,0,1,2]].cpu().numpy()).as_matrix() for x in mu]), 0.001);
        # plt.savefig(f"t_{i}.png")

        denoised_rotation = fn_sample(mu, scale)
        # visualize_so3_probabilities(jnp.array([jaxlie.SO3(x[[3,0,1,2]].cpu().numpy()).as_matrix() for x in denoised_rotation]), 0.001);
        # plt.savefig(f"t_{i}.png")
        # ziel * start.inv()
        relative_rotation = lietorch.SO3(denoised_rotation) * lietorch.SO3(x_t).inv()
        ...
        new_rotation = small_step_rotation * lietorch.SO3(x_t)
        x_next = new_rotation.vec()


        if i < len(ts) - 1:
            noise_level = t_next * torch.ones([x_t.shape[0],1],device=x_t.device)
            mu , scale = model(x_next, noise_level)
            denoised_rotation = fn_sample(mu, scale)

            relative_rotation = lietorch.SO3(denoised_rotation) * lietorch.SO3(x_next).inv()
...
def ddpm_sampling(model,gt_rotations,noise_schedule):
    ...
    for i in range(len(noise_schedule)):
        ...
        mu, scale = model(x_t_rotation, noise_level)
...
def eval(rng_seq, noise_schedule, model, output_dir, step, gt_rotations):
    ...
    if FLAGS.sampler == "heun":
        ...
        x_t_rotation = ddpm_sampling(model,gt_rotations,noise_schedule)
    ...
    sum_distance = minimum_distance(x_t_wxyz, gt_rotations, rotational_distance_between_pairs_dot_product)
...
def get_batch(batch, key, noise_schedule):
    ...
    def sample_vexp(q, temperature):
        ...
        unit_rotations = torch.tensor([.0,.0,.0,1.0]).cuda().repeat(q.shape[0],1)
        axis_angles = torch.vmap(lambda mu,s_single: IsotropicGaussianSO3(mu,
                                                                    s_single,
                                                                    force_small_scale=FORCE_SMALL_SCALE).sample_one_vmap(),
                                randomness="different")(unit_rotations, scale)
        noisy_rotation = (lietorch.SO3(unit_rotations) * lietorch.SO3.exp(axis_angles))
        qn = lietorch.SO3(q) * noisy_rotation


        return {'x': q, 'yn': qn.vec(), 'sn':scale}
...
def train(model, batch, optimizer):
    ...
    mu, scale = model(yn, sn.reshape(-1,1))
    ...
    q = batch['x']
    loss = (-fn(q, mu, scale)).mean() + (scale.mean()/8)
    # visualize_so3_probabilities(jnp.array([jaxlie.SO3(x[[3,0,1,2]].detach().cpu().numpy()).as_matrix() for x in mu]), 0.001);
    # plt.savefig(f"t_{0}.png")
    loss.backward()
    optimizer.step()
    return loss.item()
...
def main(_):
    ...
    if FLAGS.diffusion_type == "vexp":
        ...
        # schedule = LogUniformSchedule(1.5)
    ...
    ax.set_title(f'Noise schedule')  # Add this line to set the title
    ax.set_xlabel('Step')
    ax.set_ylabel('Noise level')
    plt.savefig(output_dir + FLAGS.dataset +'_'+ FLAGS.diffusion_type + '_noise_schedule.png')
    plt.savefig(output_dir + FLAGS.dataset +'_'+ FLAGS.diffusion_type + '_noise_schedule.pdf')
    ...
    if FLAGS.train:
        ...
        for step in tqdm(range(FLAGS.training_steps)):
            ...
            if step in [7000] or (step>1000 and step % 20000 == 0):
