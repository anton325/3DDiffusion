...
from gecco_torch.additional_metrics.metrics_so3 import c2st_gaussian, minimum_distance, rotational_distance_between_pairs
...
flags.DEFINE_string("dataset", "gaus_rot_x0_dist", "Dataset to train on. Can be 'checkerboard'.")
...
def train(model, batch, optimizer):
    ...
    mu = model(yn, sn.reshape(-1,1))

    
    q = batch['x']
    rotational_distance_loss = rotational_distance_between_pairs(q, mu)
    loss = rotational_distance_loss.mean()
    loss.backward()
    optimizer.step()
    return loss.item()
