...
flags.DEFINE_integer("test_nsamples", 10_000, "Number of samples to draw at testing time.")
...
def eval(rng_seq,noise_schedule, model, output_dir, step="eval", params=None):
    ...
    def fn_sample(x, delta_mu, s):
        return IsotropicGaussianSO3((lietorch.SO3(delta_mu.cuda()) * lietorch.SO3(x.cuda())).vec(), s, force_small_scale=True).sample()
...
def main(_):
    run = FLAGS.dataset + "_" + "eval_jaxmodel2"
    ...
    with open('/home/giese/Documents/SO3DiffusionModels/models/so3ddpm/checkerboard_vpres_model_240000_.pckl', 'rb') as file:
    ...
    #     beta = torch.linspace(0.0001, 0.04, FLAGS.n_steps)
    #     # This corresponds to alpha, it starts from 1, i.e. almost no change in the image
    #     noise_schedule = torch.cumprod(1 - beta, dim=0)
